...
async def createUser(newUser: dataModels.NewUser):
    ...
    # generatedOtp = otpFactory.create(newUser.email)
...
def verifyUser(otp: dataModels.Otp):
    if otpFactory.getOtp(otp.email) != otp.value:
        return {"error": 303}
    USERS.insert({'email': otp.email, 'name': otp.name, 'digest': hashlib.sha256(str(otp.pin).encode()).hexdigest()})
...
def withdraw(newTransaction: dataModels.NewTransaction, permitted: list = Depends(authorize)):
    if newTransaction.pots[0] not in permitted:
        raise exceptions.UserError(305)
    balance = POTS.get({'id': newTransaction.pots[0]}, ['balance'])[0][0]
